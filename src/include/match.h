...
// Math Functions for clamping and Root Mean Square
#define CLAMP100(X) std::clamp((double)X, 0.0, 100.0)
#define RMS_BEST2(X,Y,Z) CLAMP100(std::sqrt((std::pow(CLAMP100(X), 2.0) + std::pow(CLAMP100(Y), 2.0) + std::pow(CLAMP100(Z), 2.0) - std::fmin(std::fmin(std::pow(CLAMP100(X), 2.0), std::pow(CLAMP100(Y), 2.0)), std::pow(CLAMP100(Z), 2.0))) * 0.5))
...
// Helper functions
extern std::pair<int, std::string> parse_ordinal(std::string);
extern std::vector<Objid> nearby_objects(Objid);

// Match functions
extern Objid match_object(Objid player, const char *name, Var *object_list = nullptr);
extern bool token_match(Objid*, Objid, const char*, std::vector<Objid>);
extern std::vector<int> complex_match(std::string, std::vector<std::string>, int threshold = 70);
